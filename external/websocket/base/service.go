...
import (
	...
	security2 "github.com/backtesting-org/live-trading/external/websocket/security"
)
...
type BaseService struct {
	...
	// Security components
	rateLimiter *security2.RateLimiter
	validator   *security2.MessageValidator
...
func NewBaseService(config Config, logger logging.ApplicationLogger) *BaseService {
	...
	validationConfig := security2.ValidationConfig{
	...
	return &BaseService{
		config:         config,
		logger:         logger,
		rateLimiter:    security2.NewRateLimiter(config.RateLimitCapacity, config.RateLimitRefill),
		validator:      security2.NewMessageValidator(validationConfig),
		metrics:        performance2.NewMetrics(),
